...
def model_spec(x, h=None, init=False, ema=None, dropout_p=0.5, nr_resnet=5, nr_filters=160, output_dim=10*10, resnet_nonlinearity='concat_elu', energy_distance=False):
    """
    ...
    with arg_scope([nn.conv2d, nn.deconv2d, nn.gated_resnet, nn.dense], counters=counters, init=init, ema=ema, dropout_p=dropout_p):
        ...
        with arg_scope([nn.gated_resnet], nonlinearity=resnet_nonlinearity, h=h):
            ...
            if energy_distance:
                ...
                x_out = nn.nin(tf.nn.elu(ul),output_dim)
