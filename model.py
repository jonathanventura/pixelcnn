...
class BinaryModel(ModelDesc):
    ...
    def build_graph(self, images):
        ...
        # run PixelCNN model
        logits = pixelcnn(images*2.-1.,num_filters=32,num_layers=7,num_outputs=1)
...
class GrayscaleModel(ModelDesc):
    def inputs(self):
        return [tf.placeholder('uint8',(None,None,None),name='image')]

    def build_graph(self, images):
        # run PixelCNN model
        logits = pixelcnn(tf.cast(tf.expand_dims(images,-1),'float32')/255.*2.-1.,num_filters=32,num_layers=7,num_outputs=256)
        ...
        # add summaries
        summary.add_moving_summary(loss)
        tf.summary.image('image',tf.expand_dims(images,axis=-1))
        tf.summary.image('prediction',tf.expand_dims(pred,axis=-1))
